...
// ********* generate ppt *******************************************
        ...
        slide.addText('PALETTE', { x:1.5, y:0.5, fontSize:18, color:'000000' });
            ...
            slide.addText('Rooms:',                 { x:1.5, y:1.0, fontSize:18, color:'000000' });
            slide.addText(''+roomFlatDescription,   { x:1.5, y:1.2, fontSize:14, color:'000000' });

            slide.addText('People:',                { x:1.5, y:2.5, fontSize:18, color:'000000' });
            slide.addText(''+fable.palette.people,  { x:1.5, y:2.7, fontSize:14, color:'000000' });

            slide.addText('Things:',                { x:5.5, y:2.5, fontSize:18, color:'000000' });
            slide.addText(''+fable.palette.things,  { x:5.5, y:2.7, fontSize:14, color:'000000' });


        // *** compute layour for all rooms (statically) wrt the page ***
        ...
        function visitAndDo(block,currentRect,depth = 0){
            console.log( '-> ' , block.name , currentRect, block.children.length+1);
            var node = {};
            node.name = block.name;
            node.childrenNum = block.children.length;
            node.depth = depth;
            ...
            block.children.forEach(function(childBlock,index){
                visitAndDo(childBlock,rects[index+1],depth+1);
            });
        }
        visitAndDo(fable.palette.rooms.world,pageRect);
